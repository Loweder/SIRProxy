...
sub client_cleanup {
	...
	foreach my $key (keys %store_time) {
		...
		if ($client_time < $cur_time) {
			...
			v_echo("Connection $mapped timed out");
		} else {
...
#   2: Size
sub receive_data {
	v_echo("Got data from the server: UUID - '$_[0]', Size - $_[1]");
	client_revert($_[0]);
...
sub transmit_data {
	...
	v_echo("Got data to the server: UUID - '$s_addr', Size - " . length($s_data));
	$ln_primary->printf("%s %d\n", $s_addr, length($s_data));
	$ln_primary->print($s_data);
}

# Set trap for shutdown
$SIG{TERM} = $SIG{INT} = sub { exit_close("Proxy killed, exiting", $ln_primary, $ln_client); };

# Initiate sockets
my $ln_listener = socket_bind("Listening for server at", IPPROTO_TCP, $server_port, 1);
$ln_client = socket_bind("Listening for client at", IPPROTO_UDP, $client_port);
...
while (1) {
	...
	foreach my $ln_ready (@lns_ready) {
		if ($ln_ready == $ln_client) { # Data available on the client socket
			...
			exit_close("Server disconnected, exiting", $ln_primary, $ln_client) unless defined $s_meta;
